# Shipwreck (Creature Bank)

<figure markdown="span">

![Shipwreck Creature Bank](../assets/locations-shipwreck.webp){ width="475" align=right }

</figure>

___
<p style="text-align: center;" markdown>[Visitable Field](index.md#visitable-field)<br>[Creature Bank (Ⅱ-Ⅲ)](../fields/creature_bank.md)</p>
___

| Units | Reward | Extra |
| :--- | :--- | :--- |
| 4x&nbsp;[Wraiths](../units/wraiths.md) | [:morale_positive:](../keywords/morale.md#positive) and 5 :gold:. | 2 :gold: for every Stacked unit and [**Search(X)**](../keywords/search.md) the [Artifact](../artifacts/index.md) deck, where X is the number of Stacked units. |

___


## Notes

- This field is not considered a sea field.


## Fourni avec

- [Naval Battles Expansion](../content/naval_battles_expansion.md)


## Voir aussi

- [Shipwreck](shipwreck.md)
- [Liste des sites](index.md)
- [Liste des tuiles](../tiles/index.md)
