# Shrine of Magic Incantation

=== "Land"

    <figure markdown="span">
        ![Shrine of Magic Incantation Map Land Location](../assets/locations-shrine_of_magic_incantation.webp){ width="475" align=right }
    </figure>

=== "Sea"

    <figure markdown="span">
        ![Shrine of Magic Incantation Map Sea Location](../assets/locations-shrine_of_magic_incantation_water.webp){ width="475" align=right }
    </figure>

___
<p style="text-align: center;" markdown>[Visitable Field](index.md#visitable-field)</p>
___
<p style="text-align: center;" markdown>[**Search(2)**](../keywords/search.md) the [Spell](../spells/index.md) Deck.</p>
___


## Voir aussi

- [Liste des sites](index.md)
- [Liste des tuiles](../tiles/index.md)
