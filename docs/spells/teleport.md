# Teleport

![Teleport](../assets/spells-teleport.webp){ width="340" align=right }

___
<p style="text-align: center;" markdown>[Expert Water Spell](index.md#school-of-water-magic)</p>
___
<p style="text-align: center;" markdown>:activation: During Combat, move one allied \* [unit](../units/index.md) to any empty space - ignore any obstacles or effects when moving.<br><br>:power: 0 ➣ \*:bronze_tier:<br>:power: 1 ➣ \*:bronze_tier: or :silver_tier:<br>:power: 2 ➣ \*:bronze_tier: or :silver_tier: or :gold_tier:<br><br>— OR —<br><br>:instant: +1 :power:</p>
___


## Fourni avec

- [Core Game](../content/core_game.md)


## Voir aussi

- [Ecole de magie de l'Eau](index.md#school-of-water-magic)
- [List of Spells](index.md)
